...
### Recommendations
1. css imnport는 하나의 자바,타입스크립트 entry 파일에 포합 시키도록 함
2. 글로벌 스타일, Tailwind 스타일시트는 애플리케이션 루트에 import
3. Tailwind css는 일반적 디자인 패턴 유틸클래스로 지원하기 떄문에 대부분 스타일링에 사용하는게 좋음
4. css 모듈에는 일관된 명명 규칙적용
> <name>.tsx 대신 <name>.module.css 사용

### 로컬이미지
- 정적 파일은 루트 디렉토리의 PUBLIC 디렉토리에 저장할 수 있음

### blog라우팅 페이지 만들고 문서 코드에서 몇가지 수정 실습

### import방식으로 이미지 사용법 실습
- import 방식 사용할 경우 public 인식 x
- 따라서 src/image/ 디렉토리 만들고 그곳에 이미지 저장해서 사용
- 실습1에 사용한 blog 페이지에 import 방식으로 코드 추가

### import 이미지 vs public 이미지
1. 컴포넌트 내 사용하는 모든 이미지
2. 빌드 타임 경로가 확정된 이미지
3. 최적화, 성능이 가장 주요한 이미지
